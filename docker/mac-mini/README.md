...
- a valid Strapi read-only API token before you build the production `web` image
...
### 4. Create the first Strapi admin and a read-only API token

After Strapi is up on the Mac Mini:

1. open `http://127.0.0.1:1337/admin`
2. create the first administrator account
3. create a read-only API token for the public site build
4. put that token into your development machine build environment
...
```bash
STRAPI_URL=https://cms.officialdavidtaylor.com \
STRAPI_PUBLIC_URL=https://cms.officialdavidtaylor.com \
STRAPI_API_TOKEN=<read-token> \
STRAPI_FETCH_RETRIES=3 \
yarn docker:build:release web
yarn docker:push:release web
```

This tags the `web` image as `<semver>` and `latest`, using the same release version as `strapi`.
...
- the Astro site is a static build, so the published `web` image is a content snapshot built elsewhere against a live CMS
...
5. Build and push `web` against the live CMS:
   `STRAPI_URL=<build-cms-url> STRAPI_PUBLIC_URL=https://cms.officialdavidtaylor.com STRAPI_API_TOKEN=<read-token> yarn docker:build:release web && yarn docker:push:release web`
...
- changes in Strapi content do not automatically appear on `officialdavidtaylor.com`
- because the Astro site is statically built, content changes require a new `web` image build and deploy if you want the main site to reflect them
