...
The `web` service is an Astro Node server. Most pages are static, but `/my-record-collection` is rendered on demand from Strapi and backed by an in-memory cache that can be explicitly rewarmed by a Strapi webhook.
...
- Keep the MinIO console (`:9001`) private or behind your existing access layer.
...
## Webhook: Rewarm the Astro Records Cache
...
Recommended trigger set from Strapi:

- `record` create / update / delete / publish / unpublish
- `artist` update
- media updates that change record cover art
...
```text
x-records-invalidate-secret: <WEB_RECORDS_INVALIDATE_SECRET>
```
...
Protect Strapi admin with the identity-aware access layer you already run with Keycloak.

Practical options:

- put `cms.example.com/admin*` behind your reverse proxy + Keycloak integration
- use `oauth2-proxy`, Traefik ForwardAuth, or an equivalent OIDC-aware gateway backed by Keycloak
- keep the public API routes below outside that admin-only policy
