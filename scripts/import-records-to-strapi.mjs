...
function buildCollectionPath(pathname, query) {
...
async function findDocumentId(pathname, fieldName, fieldValue) {
  for (const status of [PUBLISHED_STATUS, DRAFT_STATUS]) {
    const response = await strapiRequest(
      buildCollectionPath(pathname, {
        filters: {
          [fieldName]: {
            $eq: fieldValue,
          },
    ...
    const documentId = response.data[0]?.documentId ?? null;
    if (documentId) {
      return documentId;
    }
  }
...
async function findArtistDocumentId(artistName) {
  return findDocumentId('/api/artists', 'name', artistName);
}

async function createArtist(artistName) {
  const response = await strapiRequest(
    buildCollectionPath('/api/artists', { status: PUBLISHED_STATUS }),
    {
      body: JSON.stringify({
        data: {
          name: artistName,
...
async function findExistingRecordDocumentId(slug) {
  return findDocumentId('/api/records', 'slug', slug);
}
...
async function uploadCoverAsset(coverPath) {
  ...
  formData.append(
    'files',
    new Blob([fileBuffer], { type: getMimeType(coverPath) }),
    fileName
  );
...
async function createRecord(record, artistDocumentId, coverAssetId) {
  await strapiRequest(
    buildCollectionPath('/api/records', { status: PUBLISHED_STATUS }),
    {
      body: JSON.stringify({
        data: {
...
async function importRecord(filePath) {
  ...
  if (existingDocumentId) {
    console.log(
      `Skipping ${record.slug}; record already exists (${existingDocumentId}).`
    );
    return;
  }
